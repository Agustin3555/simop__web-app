...
export const TipoOrigenFinanciamientoObraService = {
  getAll: async fields => {
  ...
  getRefs: async () => {
    ...
    return refsAdapter(
      TipoOrigenFinanciamientoObraAdapter.getRefs.output,
      response.data,
    )
  },
  ...
  create: async (data: TipoOrigenFinanciamientoObraModel.CreateEntity) => {
    const adaptedInput = TipoOrigenFinanciamientoObraAdapter.create.input(data)
...
} satisfies Service<TipoOrigenFinanciamientoObraModel.Entity>
